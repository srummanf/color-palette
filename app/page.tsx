...
export default function ColorExtractor() {
  ...
  const extractDominantColors = (imageElement: HTMLImageElement): Color[] => {
    ...
    const scale = Math.min(maxDimension / imageElement.width, maxDimension / imageElement.height)
  ...
  const isNearRuleOfThirds = (x: number, y: number, width: number, height: number): boolean => {
    ...
    return intersections.some(([ix, iy]) => Math.abs(x - ix) < tolerance && Math.abs(y - iy) < tolerance)
  }
  ...
  const selectDiverseColors = (candidates: ColorCandidate[], count: number): ColorCandidate[] => {
    ...
    for (const candidate of candidates) {
      ...
      const tooSimilar = selected.some((selected) => colorDistance(candidate, selected) < minDistance)
    ...
    while (selected.length < count && selected.length < candidates.length && currentMinDistance > 20) {
      ...
      for (const candidate of remaining) {
        ...
        const tooSimilar = selected.some((selected) => colorDistance(candidate, selected) < currentMinDistance)
  ...
  return (
    <div className={`min-h-screen bg-black text-white flex items-center justify-center p-4 ${spaceGrotesk.className}`}>
          ...
          >
            {uploadedImage ? (
              <div className="space-y-4">
                ...
                />
                <p className={`font-black text-xs tracking-wider ${jetbrainsMono.className}`}>CLICK TO CHANGE</p>
              </div>
            ) : (
              <div className="space-y-4">
        ...
        {dominantColors.length > 0 && !isProcessing && (
          <div className="space-y-8">
            <div className="text-center">
              <h2 className={`font-black text-sm mb-2 tracking-widest ${jetbrainsMono.className}`}>COLORS</h2>
              <p className={`font-bold text-xs opacity-70 ${rubik.className}`}>CLICK TO COPY</p>
            </div>
